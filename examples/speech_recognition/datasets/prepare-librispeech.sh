...
train_dir=train_100h_examples_asr
...
fairseq_root=/workspace/fairseq-uni
mkdir -p ${out_dir}
cd ${out_dir} || exit
...
python ${fairseq_root}/scripts/spm_train.py --input=data/lang_char/input.txt --vocab_size=${nbpe} --model_type=${bpemode} --model_prefix=${bpemodel} --input_sentence_size=100000000 --unk_id=3 --eos_id=2 --pad_id=1 --bos_id=-1 --character_coverage=1
#spm_encode --model=${bpemodel}.model --output_format=piece < data/lang_char/input.txt > ${encoded}
python ${fairseq_root}/scripts/spm_encode.py --model=${bpemodel}.model --output_format=piece < data/lang_char/input.txt > ${encoded}
