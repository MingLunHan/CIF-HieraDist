...
from fairseq.modules import (
    LayerNorm,
    MultiheadAttention,
    CustomizedEncDecMultiheadAttention,
)
...
# Revised by mlhan from TransformerDecoderLayerBase, which now supports direct argments
class TransformerDecoderLayerBaseDirectArgs(nn.Module):
    """Decoder layer block.
    ...
    *cfg.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        ...
        if no_encoder_attn:
            ...
            if cfg.encoder_attn_type == "customized":
                self.encoder_attn = self.build_customized_encoder_attention(
                    self.embed_dim, cfg
                )
            elif cfg.encoder_attn_type == "normal":
                self.encoder_attn = self.build_encoder_attention(self.embed_dim, cfg)
            else:
                self.encoder_attn = self.build_encoder_attention(self.embed_dim, cfg)
    ...
    def build_customized_encoder_attention(self, embed_dim, cfg):
        return CustomizedEncDecMultiheadAttention(
            embed_dim,
            cfg.decoder_attention_heads,
            kdim=cfg.encoder_embed_dim,
            vdim=cfg.encoder_embed_dim,
